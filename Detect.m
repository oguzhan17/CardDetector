% Program runs be like : Detect('Dataset/imagename.jpg')   
% Detect('Dataset/10clubs.jpg')  
% Detect('Dataset/10diamonds.jpg')   
% Detect('Dataset/10hearts.jpg')   
% Detect('Dataset/10spades.jpg')   
...
function Detect(fn)
    ...
    %% PROCESS EACH CARD
     for ii = 1:n_labels
           ...
           %If the area of convex hull is <=30000, it probably is not a playing card but
           %noise, so reject it.
          ...
          % TEMPLATE MATCHING             
            ...
            og = insertText(og,position,current_title,'FontSize',120,'BoxColor','blue','BoxOpacity',0.9,'TextColor','white','AnchorPoint','Center');
            imshow(og);
        end
